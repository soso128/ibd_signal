...
// ZBS stuff 
extern "C" void kzget1_( const char*, int&, int&, int*, long );
...
int main(int argc, char* argv[])
{
    ...
    if (argc < 3) {
        ...
        cout << "    usage: " << argv[0]
            << " output file (root)" 
            << " input root file [input zbs file] " << endl;
        cout << endl;
        return 1;
    }
    ...
    if(argc > 3){
        ...
        fort_fopen_(luni, fname_in_zbs.c_str(), ftype, rw, fname_in_zbs.length());
    }
    string fname_out = argv[1];
    ...
    for (int i = 0; i < ntotal; i++) {
        ...
        if (is_zbs){
            if (ierr != 2) skcread_(luni,ierr);
            if (ierr == 2)
            {
                cout << "End of zbs input file. " <<endl;
                nmue = -1;
            }
            if (ierr == 0){
                int ndata, isegm = 1;
                ...
                if ( zbsget.rdata[ 29 ] > 0.0001 ) 
                {
                    ...
                    bsvertex[ 0 ] = zbsget.rdata[ 0 ];
                    bsvertex[ 1 ] = zbsget.rdata[ 1 ];
                    bsvertex[ 2 ] = zbsget.rdata[ 2 ];
                }

                // *** nmue 
                int muerr = 1;
                muechk_(bsvertex, &muerr );
                nmue = apmue_.apnmue;
                //cout << "  nmue: " << nmue << endl;
            }
        }
